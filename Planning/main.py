...
from planning_config import config
...
def exec_waypoint_nav_demo(args):
    ...
    carla_process = subprocess.Popen(launch_command, stdout=subprocess.DEVNULL)
    ...
    initial_transform = carla.Transform(initial_location,
                                        carla.Rotation(yaw=\
                                        config.agent.initial_position.yaw))
    ...
    #############################################
    # Get options
    enable_live_plot = config.plot.live_plotting
    live_plot_period = float(config.plot.live_plotting_period)
    ...
    waypoints_file = config.waypoints_filename
    waypoints_filepath = os.path.join(config.root_dir, "Planning", waypoints_file)
    waypoints_np   = None
