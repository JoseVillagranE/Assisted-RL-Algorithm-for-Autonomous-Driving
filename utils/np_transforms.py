...
####### COPY FROM : https://github.com/mlagunas/pytorch-nptransforms ####################
...
class Normalize_mean_std(object):
    """
    Normalize the values of numpy array w/ mean and std normalization
    """
    ...
    def __init__(self, mean=None, std=None):
        ...
        if isinstance(mean, list):
            ...
            raise TypeError(f"Mean should be a ndarray or python list")

        if isinstance(std, list):
            std = np.array(std)
        elif not isinstance(std, np.ndarray):
            raise TypeError(f"Std should be a ndarray or python list")


        # TODO: Generalize for more dimension of mean and std
        if mean.ndim == 1:
            mean = mean[:, None, None]
        if std.ndim == 1:
            std = std[:, None, None]
    ...
    def __call__(self, pic):
        ...
        if not _is_numpy_image(pic):
            raise TypeError(f"Img should be Numpy Image. Got {type(pic)}")
        if pic.ndim != 3:
            raise ValueError(f"Expected Numpy to be a Numpy image of size (C, H, W). Got pic.ndim = {pic.ndim}")
